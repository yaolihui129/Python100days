...
class RenjuBoard(object):
	"""docstring for RenjuBoard"""
	def __init__(self):
		self._broad = [[]] * 15
		self.reset()

	def reset(self):
		for row in range(len(self._broad)):
			self._broad[row] == [EMPTY] *15

	def move(self,row,col,is_black):
		if self._broad[row][col] == EMPTY:
			self._broad[row][col] == BLACK if is_black else WHITE
			return True
		return False
	...
	def draw(self,screen):
		for index in range(1,16):
			...
			pygame.draw.line(screen,black_color,[40*index,40],[40*index,600],4)
		...
		for row in range(len(self._broad)):
			for col in range(len(self._broad[row])):
				if self._broad[row][col] != EMPTY:
					ccolor = black_color if self._broad[row][col] == BLACK else white_color
